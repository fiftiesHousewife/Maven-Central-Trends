...
const growthChartHTML = `<!DOCTYPE html>
<html lang="en">
<head>
<meta charset="UTF-8">
<meta name="viewport" content="width=device-width, initial-scale=1.0">
<title>Maven Central — Artifact Trends</title>
...
<style>
  * { margin: 0; padding: 0; box-sizing: border-box; }
  ...
  .detail-item .count { color: #3b82f6; font-size: 0.8rem; font-weight: 600; white-space: nowrap; margin-left: 0.5rem; }
  .detail-item .meta { font-size: 0.75rem; color: #94a3b8; }
  .detail-item .badges { display: flex; gap: 0.3rem; flex-wrap: wrap; }
  .detail-item .badge {
    display: inline-block; font-size: 0.7rem; padding: 1px 6px;
    border-radius: 3px; background: #334155; color: #94a3b8;
  }
  .detail-item .badge.license { background: #1e3a5f; color: #60a5fa; }
</style>
</head>
<body>
<div class="chart-wrap">
  <h1>New Artifacts Per Month</h1>
  <p id="status">Loading data...</p>
  <p id="sub">Artifacts added to Maven Central each month across all groups, with cumulative total. Outlier groups (500+ artifacts) are capped. Click any bar for details.</p>
  <div id="chart"></div>
</div>
...
const milestones = [
  ...
  { month: '2024-06', label: 'Sonnet 3.5', color: '#8b5cf6' },
  { month: '2024-08', label: 'Cursor AI', color: '#06b6d4' },
  { month: '2025-02', label: 'Sonnet 3.7', color: '#8b5cf6' },
...
function showDetail(month) {
  fetch('/api/new-groups/details?month=' + month)
    ...
    .then(groups => {
      ...
      groups.sort((a, b) => (b.artifact_count || 0) - (a.artifact_count || 0));
      const totalArt = groups.reduce((s, g) => s + (g.artifact_count || 0), 0);
      document.getElementById('detail-title').textContent =
        month + ' — ' + groups.length + ' groups, ' + totalArt.toLocaleString() + ' artifacts';
      document.getElementById('detail-list').innerHTML = groups.map(g => {
        ...
        if (g.artifact_count > 0) badges.push('<span class="badge">' + g.artifact_count + ' artifacts</span>');
        ...
        return '<div class="detail-item">' +
          ...
          '<span class="count">' + (g.artifact_count || 0) + ' art</span></div>' +
          (badges.length ? '<div class="badges">' + badges.join('') + '</div>' : '') +
          '<div class="meta">' + g.first_artifact + ' — ' + g.first_published + '</div>' +
          '</div>';
      }).join('');
...
function update() {
  fetch('/api/growth')
    ...
    .then(data => {
      ...
      const cumulArt = data.map(d => d.cumul_artifacts);
      ...
      const avgNew = Math.round(recent6.reduce((s, d) => s + d.new_artifacts, 0) / recent6.length);

      document.getElementById('status').textContent =
        last.cumul_artifacts.toLocaleString() + ' total artifacts tracked';
      document.getElementById('summary').innerHTML =
        '<div class="stat-card"><div class="value blue">' + last.cumul_artifacts.toLocaleString() + '</div><div class="label">Total artifacts</div></div>' +
        '<div class="stat-card"><div class="value amber">' + avgNew.toLocaleString() + '</div><div class="label">Avg new artifacts/month (6m)</div></div>' +
        '<div class="stat-card"><div class="value green">' + newArt[newArt.length - 1].toLocaleString() + '</div><div class="label">Last month</div></div>';
...
func ArtifactChart(w http.ResponseWriter, r *http.Request) {
	w.Header().Set("Content-Type", "text/html; charset=utf-8")
	w.Write([]byte(growthChartHTML))
}
